            ...
            try:
                ...
                try :
                    ...
                    if r is not None and 'dmsg' in r:
                        dmsg = dmsg_value = r.get('dmsg')
                    emsg = None
                    if r is not None and 'emsg' in r:
                        emsg = emsg_value = r.get('emsg')
...
class Diu (BaseIU):
    ...
    def download_data_parallel(self, symbol_list, output_directory, tf):
        if self.use_pool:
            logger.info (f'Fetching data in parallel...')
...
class Tiu (BaseIU):
    ...
    def update_holdings (self):
        ...
        if ret is not None and isinstance (ret, list):
            ...
            if len (stocks):
                ...
                logger.info (f'No holdings to update')
    ...
    def square_off_position (self, order_details:list):
        ...
        if r is not None and isinstance(r, list):
            ...
            if df is not None:
                ...
                logger.info (f'get_order_book Failed, Check Manually')
        else:
            logger.info (f'get_order_book Failed, Check Manually')
            return
    ...
    def place_exit_order (self, stock:str, qty_per:float=100.0):
        ...
        if qty is None and posn_df is not None:
            logger.info (f'Checking in positions')
            df = posn_df
            try:
                ix = df[df['TSYM'] == tsym].index[0]
                qty = df.loc[ix].at['SELLABLEQTY']
        ...
        if qty is not None and qty > 0:
            ...
            if qty:
                ...
                logger.info (f'Did not place order as qty was 0.')
        else:
    ...
    def place_mm_fc_gtt_order (self, stock_details:dict, fixed_amount:float=0.0, ot='B', trade_seq=1):
        order_id = None

        fv = self.fv
        stock = stock_details['Symbol']
        dh = stock_details['High']
        st = stock_details['ST']
        dl = stock_details['Low']
        ...
        if token is not None and tsym is not None:
            qty = int (fixed_amount/dh)
            if (qty % 2): qty += 1
            remarks = 'Id_'
            remarks += str(stock_details['Strategy_id'])
            remarks += '_'
            remarks += re.sub ("[-,&]+", "_", tsym)
            ...
            if qty:
                qty1 = round(qty/4)
                qty1 = 1
                logger.info (f'placing {ot} order {stock} tsym:{tsym} pdh: {dh} qty: {qty1}')

                if trade_seq == 1:
                    if ot == 'B':
                        alert_price = app_utils.round_stock_prec(dh)
                        tp = app_utils.round_stock_prec(dh+0.1)
                        price = app_utils.round_stock_prec(tp+0.1)
                        cond = 'LTP_ABOVE_ALERT_PRICE'
                    else :
                        alert_price = app_utils.round_stock_prec(dl)
                        tp = app_utils.round_stock_prec(dl-0.1)
                        price = app_utils.round_stock_prec(tp-0.1)
                        cond = 'LTP_BELOW_ALERT_PRICE'
                    r = self.fv.place_gtt_order (buy_or_sell=ot, alert_cond=cond,
                                                 product_type='I', exchange='NSE', tradingsymbol=tsym,
                                                 quantity=qty1, discloseqty=0, price_type='SL-LMT', 
                                                 price = price, alert_price=alert_price, 
                                                 trigger_price = tp, 
                                                 remarks = remarks)
                    ...
                    if r is not None:
                        if r['stat'] == 'Not_Ok':
                            ...
                            logger.info (f'Place order success:: order id  : {r["al_id"]}')
                            remarks += "_" 
                            remarks += r["al_id"]
                            # Updating the Remarks with the Alert ID
                            r = self.fv.modify_gtt_order (buy_or_sell=ot, alert_cond=cond,
                                                 product_type='I', exchange='NSE', tradingsymbol=tsym,
                                                 quantity=qty1, discloseqty=0, price_type='SL-LMT', 
                                                 price = price, alert_price=alert_price, 
                                                 trigger_price = tp, al_id=r["al_id"],
                                                 remarks = remarks)
